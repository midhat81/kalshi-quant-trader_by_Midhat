...
import { Activity, ArrowDownRight, ArrowUpRight, BarChart3, RefreshCw, ShieldCheck, Zap } from "lucide-react";
...
export function Dashboard() {
  ...
  const maxPosition = useMemo(() => positions.reduce((m, p) => Math.max(m, Math.abs(p.market_value ?? 0)), 0), [positions]);
  ...
  return (
    <div className="terminal-grid -mx-5 -my-7 min-h-[calc(100vh-64px)] px-5 py-7 sm:-mx-7 sm:px-7 lg:-mx-9 lg:-my-8 lg:px-9 lg:py-8">
      <div className="mx-auto max-w-[1540px] space-y-4">
        ...
        <section className="grid border border-border bg-surface lg:grid-cols-[1.5fr_1fr_1fr_1fr]">
          ...
          <div className="border-b border-border p-5 lg:border-b-0 lg:border-r"><Tag>Exposure</Tag><div className="mt-3 font-mono text-xl tabular-nums">{usd(exposure)}</div><div className="mt-1 text-[10px] text-muted">capital deployed</div></div>
        ...
        <section className="grid border border-border bg-surface md:grid-cols-3">
          ...
          <div className="p-4"><Tag>System</Tag><div className="mt-2 flex items-center gap-2 text-sm"><span className="h-1.5 w-1.5 rounded-full bg-accent"/>Connected</div><div className="mt-1 font-mono text-[9px] text-muted">{lastUpdated ? `SYNC ${lastUpdated.toLocaleTimeString(undefined, { hour12: false })}` : "SYNCING"}</div></div>
        </section>
      </div>
    </div>
  );
